class ExamSchedule < ApplicationRecord
  ...
  def check_time_available
    errors.clear
    checking_schedules = ExamSchedule.where(exam_id: self.exam_id, room_id: self.room_id).where('start < ?', self.time_combine[:end]).where('finish > ?', self.time_combine[:start]).where.not(id: self&.id)
